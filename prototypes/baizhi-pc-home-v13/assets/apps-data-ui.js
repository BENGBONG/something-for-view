window.AppsDataUI = (() => {
  ...
  function renderDetail() {
    ...
    if(state.tab==='data'){
      ...
      const tableNav=`<aside class="apps-sheet-nav"><div class="apps-sheet-nav-heading">数据表 <span>${a.tables.length}</span></div><nav aria-label="应用内数据表">${a.tables.map(t=>`<button aria-pressed="${t.id===table.id}" class="${t.id===table.id?'active':''}" data-app-table="${t.id}"><svg viewBox="0 0 16 16" fill="none" aria-hidden="true"><rect x="2" y="2.5" width="12" height="11" rx="1.5"/><path d="M2 6.5h12M6 6.5v7"/></svg><span title="${esc(t.name)}">${esc(t.name)}</span><em>${M.rows(state.user,a.id,t.id,state.scope,personal()).length}</em></button>`).join('')}</nav></aside>`;
      body=`<div class="apps-data-layout">${tableNav}<section class="apps-sheet-content" aria-label="${esc(table.name)}"><div class="apps-sheet-heading"><strong>${esc(table.name)}</strong><span class="apps-caption">只读查看</span></div><div class="apps-toolbar"><div class="apps-filter-group">${ownerFilter}${state.scope==='all'?`<select data-app-filter="department" aria-label="部门">${opts([['','全部部门'],...[...new Set(all.map(r=>M.users[r.owner].department))].map(d=>[d,d])],state.department)}</select>`:'<span class="apps-caption">数据由 Agent 持续更新</span>'}</div><input type="search" aria-label="搜索表格数据" data-app-filter="search" placeholder="搜索表格内容" value="${esc(state.search)}"></div><div class="apps-table-scroll apps-grid-scroll"><table class="apps-table apps-data-grid" style="width:${44+widths.reduce((n,w)=>n+w,0)+170+158}px"><colgroup><col style="width:44px">${widths.map(w=>`<col style="width:${w}px">`).join('')}<col style="width:170px"><col style="width:158px"></colgroup><thead><tr><th scope="col">#</th>${table.fields.map(f=>`<th scope="col">${esc(f[0])}<small>${esc(f[1])}</small></th>`).join('')}<th scope="col">数据所属人</th><th scope="col">更新时间</th></tr></thead><tbody>${slice.map((r,i)=>`<tr><td>${(state.page-1)*7+i+1}</td>${r.values.map((v,j)=>`<td class="${/数值|整数/.test(table.fields[j]?.[1])?'apps-number-cell':''}">${value(v)}</td>`).join('')}<td>${person(r.owner)}</td><td class="apps-time-cell">${esc(r.updated)}</td></tr>`).join('')}</tbody></table>${!rows.length?empty('当前范围暂无记录',state.search||state.owner||state.department?'请调整筛选条件':'Agent 更新这张表后，可在这里查看。'):''}</div>${footer}</section></div>`;
    }else{
    ...
    root.innerHTML=head(a.name,a.description)+`${scopeButtons?`<div class="apps-scope">${scopeButtons}${state.scope==='all'?'<span class="apps-caption">当前企业内授权数据</span>':''}</div>`:''}<div class="apps-segment"><button data-app-tab="data" class="${state.tab==='data'?'active':''}">数据表</button><button data-app-tab="history" class="${state.tab==='history'?'active':''}">更新历史</button></div>${body}<p class="apps-caption">最近读取：${new Date().toLocaleString('zh-CN',{hour12:false})} · 当前为已提交数据</p>`;
  }
