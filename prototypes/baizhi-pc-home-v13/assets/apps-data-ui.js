window.AppsDataUI = (() => {
  const M=AppsDataModel, q=s=>document.querySelector(s);
  const esc=x=>String(x??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const value=x=>x===null||x===undefined?'—':typeof x==='boolean'?(x?'是':'否'):typeof x==='number'?x.toLocaleString('zh-CN'):esc(x);
  ...
  const resetFilters=()=>Object.assign(state,{search:'',owner:'',department:'',agent:'',operation:'',historyTable:'',from:'',to:'',page:1});
  function navigate(id=null,scope=null,tab='data') {
    if(ArtifactUI.guard(()=>navigate(id,scope,tab)))return;
  ...
  function renderList() {
    ...
    root.innerHTML=head('应用数据','查看 Agent 更新的业务数据与变更记录')+`<div class="apps-toolbar"><span class="apps-caption">${M.apps.some(a=>M.canManage(state.user,a,personal()))?'展示已使用及有权管理的应用':'仅展示我的数据，成功更新后自动加入应用'}</span><input type="search" data-app-filter="search" aria-label="搜索应用" placeholder="搜索应用名称" value="${esc(state.search)}"></div><div class="apps-panel">${list.length?`<div class="apps-table-scroll"><table class="apps-table"><thead><tr><th>应用名称</th><th>数据表</th><th>数据条数</th><th>最近更新时间</th><th>最近更新人</th></tr></thead><tbody>${list.map(a=>{
      const scope=initialScope(a),h=M.history(state.user,a.id,scope,personal()),total=a.tables.reduce((n,t)=>n+M.rows(state.user,a.id,t.id,scope,personal()).length,0);
  ...
  function renderDetail() {
    ...
    let body='';
    const fullHistory=M.history(state.user,a.id,state.scope,personal());
    const ownerIds=[...new Set(state.tab==='history'?fullHistory.map(r=>r.actor):M.rows(state.user,a.id,table.id,state.scope,personal()).map(r=>r.owner))];
    const ownerFilter=state.scope==='all'?`<select aria-label="${state.tab==='history'?'触发人':'数据所属人'}" data-app-filter="owner">${opts([['',state.tab==='history'?'全部触发人':'全部所属人'],...ownerIds.map(id=>[id,M.users[id].name])],state.owner)}</select>`:'';
    if(state.tab==='data'){
      const all=M.rows(state.user,a.id,table.id,state.scope,personal());
      const rows=all.filter(r=>(!state.owner||r.owner===state.owner)&&(!state.department||M.users[r.owner].department===state.department)&&(!state.search||r.values.some(v=>String(v).includes(state.search))));
      const footer=pageFooter(rows.length),slice=rows.slice((state.page-1)*7,state.page*7);
      body=`<div class="apps-toolbar"><div class="apps-filter-group">${ownerFilter}${state.scope==='all'?`<select data-app-filter="department" aria-label="部门">${opts([['','全部部门'],...[...new Set(all.map(r=>M.users[r.owner].department))].map(d=>[d,d])],state.department)}</select>`:'<span class="apps-caption">只读查看 · 数据由 Agent 持续更新</span>'}</div><input type="search" aria-label="搜索表格数据" data-app-filter="search" placeholder="搜索表格内容" value="${esc(state.search)}"></div><div class="apps-panel"><div class="apps-sheet-tabs" role="tablist" aria-label="数据表">${a.tables.map(t=>`<button role="tab" aria-selected="${t.id===table.id}" class="${t.id===table.id?'active':''}" data-app-table="${t.id}">▦ ${esc(t.name)}<em>${M.rows(state.user,a.id,t.id,state.scope,personal()).length}</em></button>`).join('')}</div><div class="apps-table-scroll"><table class="apps-table"><thead><tr><th>#</th>${table.fields.map(f=>`<th>${esc(f[0])}<small>${esc(f[1])}</small></th>`).join('')}<th>数据所属人</th><th>更新时间</th></tr></thead><tbody>${slice.map((r,i)=>`<tr><td>${(state.page-1)*7+i+1}</td>${r.values.map(v=>`<td class="${String(v).length>14?'apps-long-cell':''}">${value(v)}</td>`).join('')}<td>${person(r.owner)}</td><td>${esc(r.updated)}</td></tr>`).join('')}</tbody></table>${!rows.length?empty('当前范围暂无记录',state.search||state.owner||state.department?'请调整筛选条件':'Agent 更新这张表后，可在这里查看。'):''}</div>${footer}</div>`;
    }else{
    ...
    root.innerHTML=head(a.name,a.description)+`<div class="apps-scope"><span>数据范围</span><button data-app-scope="mine" class="${state.scope==='mine'?'active':''}" ${!M.allowed(state.user,a,'mine',personal())?'disabled':''}>我的数据</button>${M.canManage(state.user,a,personal())?`<button data-app-scope="all" class="${state.scope==='all'?'active':''}">全部数据</button>`:''}<span class="apps-caption">${state.scope==='all'?'当前企业内授权数据 · ':''}应用创建者：${esc(M.users[a.creator].name)}</span></div><div class="apps-segment"><button data-app-tab="data" class="${state.tab==='data'?'active':''}">数据表</button><button data-app-tab="history" class="${state.tab==='history'?'active':''}">更新历史</button></div>${body}<p class="apps-caption">最近读取：${new Date().toLocaleString('zh-CN',{hour12:false})} · 当前为已提交数据</p>`;
  }
  function fingerprint() {
    const items=state.app?[app()]:availableApps();
    return JSON.stringify(items.filter(Boolean).map(a=>[a.id,M.history(state.user,a.id,state.app?state.scope:initialScope(a),personal())]));
  }
