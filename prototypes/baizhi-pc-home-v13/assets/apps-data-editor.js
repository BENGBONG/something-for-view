...
window.AppsDataEditor = (() => {
  ...
  const permissions=row=>M.permissions(context().user,context().appId,context().table,row,context().scope,context().personal);
  ...
  function actions(row){
    ...
    return `<td class="apps-row-operation">${p.delete?`<div class="apps-row-actions"><button class="apps-trash" data-delete-row="${row.id}" aria-label="删除 ${esc(row.values[0])}" title="删除">${icon}</button></div>`:p.update?'<span class="apps-caption">—</span>':`<span class="apps-row-readonly" title="${row.owner!==context().user?'只能维护本人记录':'这张表未开放人工修改'}">只读</span>`}</td>`;
  }
  ...
  function start(id,field=0){
    ...
    const c=context(),table=definition(c),row=id?M.rows(c.user,c.appId,c.table,c.scope,c.personal).find(r=>r.id===id):null;
  ...
  function askDelete(id){
    ...
    const c=context(),row=M.rows(c.user,c.appId,c.table,c.scope,c.personal).find(r=>r.id===id);
