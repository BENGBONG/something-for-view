...
window.ArtifactUI = (() => {
  ...
  function configureFolder() {
    ...
    if (artifact) {
      q('#knowledge-folder-title').textContent = parts[2] || 'Agent 产物';
  ...
  function sync() {
    ...
    ['personal','enterprise'].forEach(space => {
      ...
      q(`#artifact-tree-${space}`).innerHTML = `<button class="side-sub-item" data-artifact-folder="${model.folder(space)}"><svg class="icon"><use href="#ico-folder"/></svg>Agent 产物<span class="artifact-system">系统</span></button><div class="knowledge-tree-children">${agents.map(agent => `<button class="side-sub-item" data-artifact-folder="${esc(model.folder(space,agent))}"><svg class="icon"><use href="#ico-folder"/></svg><span class="tree-folder-name">${esc(agent)}</span></button>`).join('')}</div>`;
    });
  }
  function init() {
    ...
    q('#knowledge-side-list').addEventListener('click', event => { const button = event.target.closest('[data-artifact-folder]'); if (button) openKnowledgeFolder(button.dataset.artifactFolder, button); });
